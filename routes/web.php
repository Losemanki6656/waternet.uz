...
Route::middleware(['auth'])->group(function () {
    ...
    // ── Available to ALL authenticated users (no section permission) ───
    ...
    // ── Super-admin only (controller enforces auth()->user()->id == 1) ──
    ...
    // ══════════════════════════════════════════════════════════════
    // Section routes — protected by Spatie permission middleware
    // ══════════════════════════════════════════════════════════════

    // ── Dashboard (bosh-menu) ─────────────────────────────────────
    Route::middleware('permission:bosh-menu')->group(function () {
    ...
    Route::middleware('permission:clients')->group(function () {
        ...
        Route::get('/update-per-page',[App\Http\Controllers\HomeController::class, 'update_per_page'])->name('update_per_page');
        ...
        Route::get('/location-update',     [App\Http\Controllers\HomeController::class, 'update_location'])->name('update_location');
    ...
    // ── Client Export ─────────────────────────────────────────────
    Route::middleware('permission:ClientExport')->group(function () {
    ...
    // ── Warehouse (sklad) ─────────────────────────────────────────
    Route::middleware('permission:sklad')->group(function () {
    ...
    Route::middleware('permission:users')->group(function () {
        ...
        Route::post('/users/update/{id}', [App\Http\Controllers\UserController::class, 'update'])->name('user_update');
