...
@section('content')
                    ...
                    @foreach ($orders as $order)
                        <tr class="row1" data-id="{{ $order->id }}">
                            <td class="text-center">
                                <i class="fas fa-stream" style="color: blue; cursor: pointer"></i></td>
                            <td class="text-center">
                                <span onclick="viewOrder({{$order}})" style="cursor: pointer">
                                    {{ $orders->currentPage() * 10 - 10 + $loop->index + 1 }}</span></td>
                                    ...
                                    <form action="{{ route('order_edit', ['id' => $order->id]) }}" method="post">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div>
                                                ...
                                                <textarea class="form-control" name="izoh" id="" rows="2">{{$order->comment}}</textarea>
                                            </div>
                                        </div>
    ...
    <div class="modal fade" id="view-order" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="myModalLabel">{{ __('messages.view_order') }}
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                </div>
...
@section('scripts')
    ...
    <script>
        ...
        {
            let city = order.client?.city?.name ?? '';
            let area = order.client?.area?.name ?? '';
            let addr = order.client?.address ?? '';
            let address = city + ', ' + area + ', ' + addr;

            $('#view_full_name').html(order.client.fullname);
            $('#view_phone').html(order.client.phone);
            $('#view_address').html(address);
            $('#view_comment').html(order.comment);
    ...
    <script>
        function onsel(id) {
            ...
            if (document.getElementById("tolov-usuli" + id).value === 3) {
                x = document.getElementById("sena" + id).value;
                y = document.getElementById("count" + id).value;
                z = x * y;
                $("#olinganpul" + id).val(z);
