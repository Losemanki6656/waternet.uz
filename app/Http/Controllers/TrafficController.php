<?php
    ...
    class TrafficController extends Controller
    {
        ...
        public function update_status_traffic($traffic, Request $request)
        {
            if($request->active_status == 'false') $status = false; else $status = true;

            Traffic::find($traffic)->update([
        ...
        public function traffics()
        {
            if (auth()->user()->id == 1) {
                ...
                $month_traffics = Traffic::where('type_traffic', 'month')->where('del_status', true)->where('active_status', true)->get();
                $year_traffics = Traffic::where('type_traffic', 'year')->where('del_status', true)->where('active_status', true)->get();
            }
